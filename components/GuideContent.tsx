...
export default function GuideContent({ cmsContent = {}, modules = [], copy = {} }: GuideContentProps) {
    ...
    const upsellItems = (cmsContent?.upsell?.items && cmsContent.upsell.items.length > 0)
        ...
        : [
            { type: 'webinar', title: c('guide.upsell_webinar_title', 'Live Webinar'), description: c('guide.upsell_webinar_desc', 'Ask questions live, see a sample registration, and network with others.'), price: c('guide.upsell_webinar_price', '₦2,500'), btnText: c('guide.upsell_webinar_btn', 'Register Now'), color: 'blue' },
            { type: 'ebook', title: c('guide.upsell_ebook_title', 'DIY E-Book'), description: c('guide.upsell_ebook_desc', 'A complete guide you can keep and revisit anytime for your business name.'), price: c('guide.upsell_ebook_price', '₦2,500'), btnText: c('guide.upsell_ebook_btn', 'Access E-Book'), color: 'orange' },
            { type: 'session', title: c('guide.upsell_session_title', 'One-on-One Session'), description: c('guide.upsell_session_desc', 'Get dedicated personal guidance from our team until your business is fully registered. No limitations, just bold results.'), price: c('guide.upsell_session_price', '₦5,000'), btnText: c('guide.upsell_session_btn', 'Secure Your Spot'), color: 'dark' },
        ];
                            ...
                            {cardUpsells.map((item: any, idx: number) => {
                                ...
                                return (
                                    <HeartCard key={idx} className="flex flex-col items-center text-center p-12 transition-transform hover:scale-[1.02] duration-500">
                                        ...
                                        <HeartButton className={`w-full ${theme.btn} ${theme.btnHover} shadow-xl ${theme.shadow} py-5 rounded-2xl`}>{item.btnText}</HeartButton>
                                    </HeartCard>
                                );
                            })}
                        </div>
                                ...
                                <div className="relative z-10 flex flex-col lg:flex-row items-center gap-12">
                                    ...
                                    <div className="flex-1 text-center lg:text-left">
                                        ...
                                        <div className="flex flex-wrap items-center justify-center lg:justify-start gap-8 md:gap-12">
                                            ...
                                            <HeartButton variant="primary" className="bg-white text-black hover:bg-gray-100 shadow-white/10 px-12 py-6 rounded-[1.8rem] transition-all font-black text-lg">{darkUpsell.btnText}</HeartButton>
                                        </div>
                                    </div>
                                </div>
                            </div>
